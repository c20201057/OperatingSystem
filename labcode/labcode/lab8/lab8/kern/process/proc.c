...
load_icode(int fd, int argc, char **kargv)
{
    ...
    // (3) 将二进制程序的TEXT/DATA/BSS部分复制到进程的内存空间
    struct Page *page = NULL;;
    
    // (3.1) 读取文件中的原始数据内容并解析elf文件头
    ...
    // (3.2) 根据elf文件头的信息读取文件中的原始数据并解析程序头
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum++) {
        ...
        // (3.3) 调用mm_map建立TEXT/DATA相关的虚拟内存区域
        ...
        // (3.4) 调用pgdir_alloc_page为TEXT/DATA分配页面，读取文件中的内容并将它们复制到新分配的页面中
        ...
        // (3.5) 调用pgdir_alloc_page为BSS分配页面，使用memset清零这些页面
    ...
    // (5) 设置当前进程的mm、cr3，重置pgdir（使用lsatp宏）
    mm_count_inc(mm);
    current->mm = mm;
